...
def dir_threshold(img, sobel_kernel=3, thresh=(0, np.pi/2)):
    ...
    absgraddir = np.arctan2(sobel_y, sobel_x)
...
def pipeline(img):
    ...
    gradx = abs_sobel_threshold(img, orient='x', sobel_kernel=KSIZE, thresh=X_THRESH)
    grady = abs_sobel_threshold(img, orient='y', sobel_kernel=KSIZE, thresh=Y_THRESH)
    #mag = mag_threshold(img, sobel_kernel=KSIZE, mag_thresh=MAG_THRESH)
    #dir = dir_threshold(img, sobel_kernel=KSIZE, thresh=DIR_THRESH)
    saturation = saturation_threshold(img, SAT_THRESH)
    red = red_threshold(img, RED_THRESH)

    # Plot gradx and saturation transformations on green and blue channels
    gb_gradx_saturation = np.dstack(( np.zeros_like(gradx), gradx, saturation)) * 255
